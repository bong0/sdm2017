...
/**
 * Base Class for implementing the Shuffle&Sort Task
 * Defines member variables
 * The Shuffle&Sort-Task is executed during the shuffle-phase and invokes the shuffle and sort operators
 * 
 * @author melhindi
 *
 */
public class ShuffleSortTaskBase extends MapReduceTask {
	...
	protected Map<Integer, String> nodeMap; // Map passed through to the shuffle operator, defines connection information to peers
	protected int port; // Port which shuffle operator will listen to, is determined from the nodeMap
	protected int partitionColumn;  // Index of the column to use for re-partitioning during shuffling and for sorting
	 ...
	 */
	public ShuffleSortTaskBase(HeapTable input, HeapTable output, int nodeId, Map<Integer, String> nodeMap, int partitionColumn, int numReducers) {
		super(input,output);
