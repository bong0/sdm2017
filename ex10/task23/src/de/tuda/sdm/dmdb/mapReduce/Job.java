...
/**
 * Similar to https://github.com/apache/hadoop/blob/trunk/hadoop-mapreduce-project/hadoop-mapreduce-client/hadoop-mapreduce-client-core/src/main/java/org/apache/hadoop/mapreduce/Job.java
 * 
 * The job executors's view of the Job.
 * 
 * It allows the user to configure the
 * job, submit it, control its execution, and query the state. The set methods
 * only work until the job is submitted
 * 
 * <p>
...
public class Job {
	...
	protected AbstractRecord outputPrototype; // used by the executor to init the ouput, must correspond to what is expected by the reducer

	protected MapReduceExecutor executor; // the executer to whoem the job will be submitted

	protected int state = 0;

	protected Class<? extends Mapper<? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue>> mapperClass;
	protected Class<? extends Reducer<? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue>> reducerClass;
	...
	public Job(Configuration conf, String name) {
	 ...
	 /**
	   * Creates a new {@link Job} with a given jobName.
	   * A Cluster will be created from the conf parameter only when it's needed.
	   *
	   * The <code>Job</code> modifies the <code>Configuration</code> so 
	   * that any changes are visible to the user.
	   * 
	...
	public static Job getInstance(Configuration conf, String name) {
	   ...
	   * The executor method blocks, at the moment, so this method will also block
	   */
	...
	public Class<? extends Mapper<? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue>> getMapperClass() {
		return mapperClass;
	}

	public void setMapperClass(
			Class<? extends Mapper<? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue>> mapperClass) {
		this.mapperClass = mapperClass;
	}

	public Class<? extends Reducer<? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue>> getReducerClass() {
		return reducerClass;
	}

	public void setReducerClass(
			Class<? extends Reducer<? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue>> reducerClass) {
