...
import de.tuda.sdm.dmdb.mapReduce.task.exercise.MapperTask;
import de.tuda.sdm.dmdb.mapReduce.task.exercise.ReducerTask;
...
public class MultiPhaseExecutor extends MapReduceExecutor{
	...
	public void submit(Job job) {
		...
		for (HeapTable partition : job.inputs) {
			...
			Future<?> future = cluster.submit(new MapperTask(partition, mapperResult, job.getMapperClass()));
			tasks.add(future);
		}
