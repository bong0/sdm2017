...
/**
 * Base Executor class that is extended by other executors
 * It prepares and creates the environment required to run the map-reduce job
 * The init() method must be called to prepare the execution of job
 * @author melhindi
 *
 */
public abstract class MapReduceExecutor {
	protected int numMappers; // number of mappers used, usually same as number of input-partitions passed in by the user
	protected int numReducers; // number of reducers used, can be ignored by different executor implementations
	protected ExecutorService cluster; // java thread pool used as cluster
	protected List<Future<?>> tasks; // each Future for a specific thread submitted to the pool, used to track task/phase completion
	...
	/**
	 * Initializes member variables and thus prepare the execution environment
	 * The number of mappers and reducers is determine based on information in the job
	...
	protected void init(Job job) {
		...
		if (0 == this.numReducers) { // second: if value was not set by user, we default to the same number as mappers
	...
	/**
	 * Wait for completion of all tasks (= usually threads) in the cluster
	 * Current implementation is very simplified 
	 */
