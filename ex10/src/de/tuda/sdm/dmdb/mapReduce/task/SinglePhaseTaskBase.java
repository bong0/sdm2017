package de.tuda.sdm.dmdb.mapReduce.task;
...
import de.tuda.sdm.dmdb.mapReduce.task.MapReduceTask;
import de.tuda.sdm.dmdb.storage.AbstractRecord;
import de.tuda.sdm.dmdb.storage.types.AbstractSQLValue;

/**
 * Base Class for implementing the MapReduce Task as an operator chain
 * Defines member variables
 * This task chains Scan->mapper->shuffle->sort->reducer operators to complete a map-reduce job
 * 
 * @author melhindi
 *
 */
public class SinglePhaseTaskBase extends MapReduceTask {
	...
	protected Class<? extends ReducerBase<? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue, ? extends AbstractSQLValue>> reducerClass;
	 ...
	 * @param nodeMap - Map passed through to the shuffle operator, defines connection information to peers
	 * @param partitionColumn - Index of the column to use for re-partitioning during shuffling and for sorting
	 ...
	 * @param reducerClass - The java Class of the reducer to instantiate and use during the reduce-phase
	 */
	public SinglePhaseTaskBase(HeapTable input, HeapTable output, int nodeId, Map<Integer, String> nodeMap, int partitionColumn,
		...
		this.recordComparator = new Comparator<AbstractRecord>() {
